...
>
> ✅ **Sprint 17 resolved (18.04.2026)**: (1) C-suite SELL → observation mode (abff1c9, 5dc2a36). (2) Director BUY boost ×1.15 (e07bbc2). (3) H6 control group fix (e07bbc2). (4) H1 cluster_buy_vs_single_buy direct test (e07bbc2 + f69cfa8). (5) Production 10b5-1 parser audit verified. Sprint 18 candidates: ~~INSIDER_CLUSTER BUY disable~~ ✅ TASK-09 23.04.2026, C-suite regex ujednolicenie, d=None bug w `_direct_cluster_vs_single`, report_generator renderowanie nowych sub_groups.
>
...
- Sprint 18 candidates post-V5: ~~INSIDER_CLUSTER BUY disable~~ ✅ done TASK-09 23.04.2026, C-suite regex ujednolicenie (`form4.pipeline.ts:158` inline `hasCsuite` regex `/\bChief\b/` vs strict `isCsuiteRole()` whitelist — TASK-10 na liście Sprint 18), d=None bug w `_direct_cluster_vs_single` (JSON zapisuje None zamiast wyliczonych wartości), report_generator nie renderuje `hc_vs_ctrl_direct` ani `cluster_buy_vs_single_buy` (sekcje w JSON, brak w markdown)
