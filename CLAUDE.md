...
- Sprint 18 candidates post-V5: INSIDER_CLUSTER BUY disable (V5 cluster vs solo p>0.37), C-suite regex ujednolicenie (`form4.pipeline.ts:158` inline `hasCsuite` regex `/\bChief\b/` vs strict `isCsuiteRole()` whitelist — TASK-10 na liście Sprint 18), d=None bug w `_direct_cluster_vs_single` (JSON zapisuje None zamiast wyliczonych wartości), report_generator nie renderuje `hc_vs_ctrl_direct` ani `cluster_buy_vs_single_buy` (sekcje w JSON, brak w markdown)
...
## System totals (23.04.2026, po TASK-07)
