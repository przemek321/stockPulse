...
> 📝 **23.04.2026 TASK-12 report_generator render direct-comparison sub_groups**: V5 backtest (e07bbc2 + f69cfa8) dodał 2 direct-comparison sub_groups ze schemą `n_a/n_b` zamiast standardowego `n`: `cluster_buy_vs_single_buy` (H1) i `hc_vs_ctrl_direct` (H6). `generate_report` w `report_generator.py:118-125` skipował je silent przez `if sub_n == 0: continue` — markdown report nie zawierał wniosków: HC SŁABSZY niż control SELL (d=-0.14 30d p=0.0162 ✓), cluster BUY vs solo BUY p>0.37. **Fix:** 3 nowe helpery — `_direct_comparison_meta` (schema detection), `_direct_comparison_sig` (strict Bonferroni → Bonferroni → 005), `_direct_comparison_table` (8-kol tabela z format `:.4f` p-value bo analyzer linia 486 zapisuje raw float). Main loop guard PRZED `sub_n == 0 skip`, routing do direct renderer. `from __future__ import annotations` dla Python 3.8 compat. **Regenerate:** `backtest_report.md` linia 72 (cluster_buy_vs_single_buy), linia 346 (hc_vs_ctrl_direct). 13 nowych testów (`tests/test_report_generator.py` — meta/sig/table/note/integration/fallback). Python testy total: 28 (15 analyzer + 13 report_generator). **Sprint 18 complete — 12/12 tasków done (TASK-01..12).**

> 🧪 **23.04.2026 TASK-11 d=None bug — verified stale**: Cheatsheet linia 327 + CLAUDE.md Sprint 18 candidates twierdziły że `scripts/backtest/analyzer.py::_direct_cluster_vs_single` zapisuje `None` zamiast cohens_d dla cluster_buy_vs_single_buy. Faktyczny inspect JSON: `backtest_results.json` ma realne d values +0.217 / -0.099 / -0.013 / -0.229 (1d/3d/7d/30d). Commit `f69cfa8` message sam cytował "d [-0.23,+0.22]" — stale doc entry sprzed regenerate (18.04.2026). Kod `analyzer.py:473-487` poprawny (pooled_sd + `float()` serialization + `note="insufficient N"` guard dla N<3). **Działania:** (1) regression test `test_v5_production_scenario_d_not_none` z fixture N=21/49 (odpowiednik prod V5) blokuje regresję — 15/15 pass w `scripts/backtest/tests/test_analyzer.py`. (2) cheatsheet linia 327 i sekcja "Known issues" #8 zaktualizowane z faktycznymi wartościami i ✅ DONE. (3) CLAUDE.md Sprint 18 candidates — ~~d=None bug~~. **Odrzucone:** print debugging, edit `_direct_cluster_vs_single`, re-run V5 backtest — bug nie istnieje. 11/12 tasków done (TASK-01..11), zostało TASK-12 (report_generator render `hc_vs_ctrl_direct` + `cluster_buy_vs_single_buy` — Python only).

> 🧹 **23.04.2026 TASK-10 C-suite regex unification**: `form4.pipeline.ts:170` miało inline regex `/\bCEO|CFO|COO|CTO|President|Chair|Chief\b/i` w `isPureDirector` check (step 3 decision tree), niespójny z strict `isCsuiteRole()` whitelist używanym w step 4 SELL gate + step 7 BUY boost (Sprint 16b). Broad regex łapał "Vice President" (bez negative lookbehind) i wszystkie Chief X Officers (Marketing, Communications, People, Sustainability), co w co-filing Director+softRole powodowało `hasCsuite=true` → NIE pure Director → proceed. **Fix:** linia 170 → `isCsuiteRole(role)`. `C_SUITE_PATTERNS` rozszerzona o `/\bChair(man|woman|person)\b/i` + `/\bVice\s+Chair(man|woman|person)\b/i` — stary `\bChair` (prefix) też łapał Chairwoman/Chairperson, ekspansja zamyka regresję. **Change list:** Director + VP/SVP/EVP-bez-context → teraz pure Director SELL → SKIP. Director + Chief Marketing/Communications/People/Sustainability → teraz pure Director SELL → SKIP. Director + CEO/CFO/Chairman/CMO/President → bez zmiany. 26 nowych testów (`form4-pipeline-rules.spec.ts`: 6 Chair variants + 20 isPureDirector coverage), 322/322 unit pass, `tsc --noEmit` clean. 10/12 tasków done (TASK-01..10), zostało TASK-11..12 (Python backtest).
>
> 🚫 **23.04.2026 TASK-09 INSIDER_CLUSTER BUY disabled**: Decyzja Opcja A (hard disable na poziomie detekcji). V5 backtest (f69cfa8) cluster_buy_vs_single_buy N=21/49 p=0.445/0.700/0.955/0.371 (1d/3d/7d/30d) — cluster nie dodaje edge'u nad solo BUY rule (d=+0.75-0.92 V5). Fix: `detectInsiderCluster` w `correlation.service.ts` zwraca `null` gdy `dir==='positive'`; stale V4 comment ("BUY clusters d=0.47 p=0.009 nadal alertują normalnie") przepisany. SELL pozostaje observation (Sprint 15). Historic audit 90d: 0 INSIDER_CLUSTER BUY alertów → retroactive archive niepotrzebny. 6 nowych testów (correlation.spec.ts TASK-09 block), 296/296 pass. Opcja B hybrid boost odłożona na Sprint 19. 9/12 tasków done (TASK-01..09 bez 08 skipped already). Zostało: TASK-10..12.

> 📊 **23.04.2026 TASK-07 AlertDeliveryGate stats audit**: Diagnoza 48h DB (12 alertów): 11 observation + 2 sell_no_edge + 2 delivered, **0 daily_limit hits**. Limit `MAX_TELEGRAM_ALERTS_PER_SYMBOL_PER_DAY=5` hardcoded per-ticker, nie konsumowany przez raporty 8h (`sendSummary` nie zapisuje do alerts entity). Scenariusze A/B (gate bug / limit za niski) odrzucone — system działa poprawnie. **Fix scenariusz C (UX):** `SummarySchedulerService` dodaje drugi query `GROUP BY nonDeliveryReason` + sekcja _Niedostarczone (N):_ w raporcie 8h z PL etykietami ze static `REASON_LABELS` map (observation→Obserwacja, sell_no_edge/csuite_sell_no_edge/cluster_sell_no_edge→SELL (zero edge) warianty, daily_limit→Dzienny limit, telegram_failed). Raport pokazuje teraz "Łącznie: 2 (dostarczono: 0) / Niedostarczone (2): Obserwacja: 2" — od razu widać legit observation vs alarm. 11 nowych testów (`alert-delivery-gate.spec.ts` 6 cases, `summary-scheduler.spec.ts` 5 cases).
...
> ⚠️ **22.04.2026 FinBERT cleanup** (DB drop 23.04.2026 FOLLOW-8): Sekcja "Sprint 2a: FinBERT Sidecar" poniżej jest **historyczna**. FinBERT sidecar + sentiment pipeline zostały usunięte z projektu (commits 988bf03, ba45976, d3a1b5c, b3a2f2b, 4b117db). Tabele DB `sentiment_scores` (18 366 rows) + `ai_pipeline_logs` (21 616 rows) dropnięte 23.04.2026 w FOLLOW-8 (commit 49c9db5) po pre-flight verify (zero @Entity refs, zero FK). Backup: `backups/sentiment-tables-drop-2026-04-23.sql.gz`. Szczegóły w CLAUDE.md → Already resolved.

> 🔍 **22.04.2026 Audyt logów produkcji → observability gap**: 500 wpisów system_logs z 18h okna pokazywały STORED / PATTERNS_DETECTED / THROTTLED, ale NIE było widać "co stało się z alertem dalej" (channel=telegram|db_only, suppressedBy). To blokowało diagnostykę TASK-04 (HPE cascade), TASK-05 (obs gate omijany?), TASK-07 (delivery gate stats). **Fix: TASK-01 AlertDispatcherService** (commit `6191a81`) — centralny punkt dispatch z `@Logged('alerts')`, priority order suppression, refactor 5 pipeline callers (-200 LoC duplication). Test jednostkowy 10 cases. Pełen task list: [TASKS-2026-04-22.md](TASKS-2026-04-22.md).

> 🛡 **22.04.2026 TASK-02 SKIP_NON_ROLE_SELL hard skip**: Tego samego dnia audyt wykrył ASX case Chen Tien-Szu "GM, ASE Inc. Chung-Li Branch" SELL $152M → fałszywy CRITICAL "INSIDER + Unusual Options" w portalu (semi ticker observation route nie blokował korelacji — TASK-05 jest niezależnym fixem). **Fix:** Form4Pipeline decision tree krok 4b PRZED daily cap + observation gate: `transactionType==SELL AND !isCsuiteRole AND !isDirectorRole → SKIP_NON_ROLE_SELL`. Logika V4 backtest: H2 SINGLE_CSUITE SELL d=-0.002 (zero edge) → non-role SELL tym bardziej. Nowa eksportowana `isDirectorRole()`, decision tree pełny komentarz inline (8 kroków), 29 nowych testów.

> 📦 **22.04.2026 TASK-03 multi-transaction Form 4 aggregation**: Ten sam ASX case ujawnił drugi, głębszy bug: 4 Chen Tien-Szu SELL były w JEDNYM Form 4 filingu (`accessionNumber_0.._3`), ale parser emitował 4 osobne `NEW_INSIDER_TRADE` eventy. Throttle (alerts.sentAt + 15min) kasował trades 2-4 zanim dotarły do `correlation.storeSignal()` → correlation widziała tylko pierwszą $152M zamiast aggregate $247M / 530k shares. **Fix collector-level** w `sec-edgar.service.ts parseAndSaveForm4`: save all rows + groupBy (insiderName, transactionType, is10b51Plan) w obrębie filing → emit 1 event per grupa z `aggregateCount`/`aggregateTradeIds` + zsumowanymi `totalValue`/`shares`. `buildForm4Prompt` dostaje nowe pole `aggregateCount`, dodaje note "×N fills — split-fill execution (market impact management lub broker TWAP)". 12 nowych testów (`form4-aggregation.spec.ts`). Odrzucony alternative: pipeline-level 60s window + `updateSignalConviction` — race condition (trade 1 event fires przed save trades 2-4).

> 🎨 **22.04.2026 TASK-05 observation visual distinction**: Investigation zmienił scope — backend gate DZIAŁA (ASX correlated alert w DB: `delivered=false, nonDeliveryReason='observation'`, Telegram suppressed). Gap był TYLKO frontend: 3 endpointy (`/alerts`, `/outcomes`, `/timeline`) zwracały alerty bez `nonDeliveryReason`, `PriorityChip` renderował observation jako CRITICAL error color. **Fix (Opcja A only):** (1) backend `alerts.controller.ts` — expose `delivered + nonDeliveryReason` w response/timeline SQL; (2) frontend `Alert` interface + `PriorityChip` przytłumiony (default color, outlined variant, label `"${priority} (${reason-label})"`) gdy `nonDeliveryReason !== null`; (3) `SignalTimeline` PRIORITY cell przytłumiony kolor + suffix. **Odrzucono Opcja B conviction cap** — maskowałaby magnitude w DB (szkodliwe dla retrospective backtest TASK-11). 6/12 tasków done (TASK-01..05 + 08). Zostało: TASK-06..07, TASK-09..12.

> 🔁 **22.04.2026 TASK-04 CorrelationService content-hash deduplication**: HPE cascade 22.04 20:33-20:38: 7 `PATTERNS_DETECTED` w 6 min, tylko 1 Telegram alert wyszedł (PATTERN_THROTTLE 2h łapał DEDUP_SKIP). Cascade = marnowanie CPU (424ms/iter) + log clutter, nie alert spam. **Fix:** content-hash dedup w `runPatternDetection` — in-memory `Map<ticker, {hash, ts}>`, 15-min window, hash = sha256(sorted (type + signalIds)) truncated. Powtórzone detekcje z tym samym składem patternów zwracają `PATTERNS_DETECTED_DUPLICATE` bez wołania `triggerCorrelatedAlert`. **Observability gap zamknięty:** `@Logged('correlation')` na `triggerCorrelatedAlert` (wcześniej DEDUP_SKIP / SKIP_LOW_CONVICTION / dispatch actions niewidoczne w system_logs — gap w TASK-01 audycie). Exported pure `hashPatternSet()` + `shouldSkipDuplicatePatternDetection()`. 20 nowych testów. Odrzucone z propozycji: TTL cleanup signals (już jest), 30-min per-pattern rate limit (PATTERN_THROTTLE 2h wystarcza).
