...
#   T1 = BUY ≥$500K (pre-zarejestrowany sub-gate #1)   T2 = tier-2 C-suite $100-500K (od 01.09, nowa kohorta)
#   N >= 10 zdarzeń ∧ hit 7d >= 60% ∧ mediana alpha XBI >= +2pp ∧ mediana REAL (price1h→7d − 1%) > 0
#   → promocja TYLKO C-suite BUY danego tieru z discovery do delivery (🎯). Director-only zostaje w obs.
#
# Użycie:  csuite-gate.sh            (tryb cron: log + jednorazowy ping Telegram per tier gdy N>=10)
...
# Jedna linia per tier: tier|N|hit%|med_alpha|mean_alpha|med_real|mean_raw
...
SELECT t.tier, coalesce(e.n, 0), coalesce(e.hit, 0), coalesce(e.med_alpha, 0), coalesce(e.mean_alpha, 0),
       coalesce(e.med_real, 0), coalesce(e.mean_raw, 0)
...
if [ "${1:-}" = "--print" ]; then
  echo "SUB-GATE C-SUITE BUY (próg N=$GATE_N; hit≥60%, med α≥+2pp, med REAL>0)"
  echo "$SUMMARY" | while IFS='|' read -r TIER N HIT MA MEANA MR RAW; do
    echo "  $TIER: N=$N hit=${HIT}% medα=${MA}pp (śr ${MEANA}) medREAL=${MR}% raw=${RAW}%"
  done
  { cat "$SQL_CTE"; cat <<'EOSQL'
SELECT tier, symbol, to_char(first_at, 'MM-DD') AS d, n_alerts AS a, CASE WHEN delivered THEN 'DELIV' ELSE 'obs' END AS st,
       sector, round(raw7d::numeric, 2) AS raw7d, round(alpha7d::numeric, 2) AS alpha, round(gap1h::numeric, 2) AS gap1h,
       round(real_net7d::numeric, 2) AS real_net, left(roles, 55) AS roles
FROM events ORDER BY tier, first_at;
EOSQL
...
echo "$SUMMARY" | while IFS='|' read -r TIER N HIT MA MEANA MR RAW; do
  ...
  PASS_REAL=$(python3 -c "print(1 if $MR > 0 else 0)")
  VERDICT="$TIER N=$N/$GATE_N hit=${HIT}% medα=${MA}pp (śr ${MEANA}) medREAL=${MR}% raw=${RAW}% | progi: hit=$PASS_HIT α=$PASS_ALPHA REAL=$PASS_REAL"
  ...
  if [ "$N" -ge "$GATE_N" ] && [ ! -f "$STATE_FILE" ]; then
    ...
    if [ $(( PASS_HIT && PASS_ALPHA && PASS_REAL )) -eq 1 ]; then
      HEAD="✅ progi spełnione → uruchom przegląd sub-gate'u (promocja C-suite BUY $TIER z discovery)"
    else
      HEAD="❌ próg N osiągnięty, ale nie wszystkie kryteria → przegląd i decyzja"
    fi
    TEXT="📐 Sub-gate C-suite BUY $TIER: N=$N osiągnięte (to NIE sygnał). $VERDICT. $HEAD. Szczegóły: scripts/csuite-gate.sh --print"
    curl -s -m 30 -o /dev/null -X POST "https://api.telegram.org/bot${TG_TOKEN}/sendMessage" \
      --data-urlencode "chat_id=${TG_CHAT}" --data-urlencode "text=${TEXT}" \
      && echo "PINGED $(date '+%F %T') N=$N" > "$STATE_FILE" \
      && echo "$(date '+%F %T') PING $TIER wysłany (N=$N)" >> "$LOG_FILE"
  fi
done
